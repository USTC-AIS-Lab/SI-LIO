...
int frame_count = 0;
double all_time = 0.0;
double all_update_time = 0.0;
string traj_log_file;
vector<int> iter_counts;
vector<double> update_time_vec;
/**************************/
...
int main(int argc, char **argv)
{
    ...
    nh.param<string>("save_path", traj_log_file, "~/traj_log.txt"); // 雷达相对于IMU的外参R

    cout << "Lidar_type: " << p_pre->lidar_type << endl;
    cout << "save trajectory to: " << traj_log_file << endl;
    ...
    while (ros::ok())
    {
        ...
        if (sync_packages(Measures)) //把一次的IMU和LIDAR数据打包到Measures
        {
            ...
            double t_pred = omp_get_wtime();
            p_imu1->Process(Measures, kf, feats_undistort);
            double t_pred_1 = omp_get_wtime();
            cout << "predict time: " << (t_pred_1 - t_pred) * 1000 << " ms" << endl;
            ...
            double step_2_2 = omp_get_wtime();
            // cout << "feats_down time: " << (step_2_2 - step_2_1) * 1000 << " ms" << endl;
            ...
            int iter_num = 0;
            kf.update_iterated_dyn_share_modified(LASER_POINT_COV, feats_down_body, ikdtree, Nearest_Points, NUM_MAX_ITERATIONS, extrinsic_est_en, iter_num);
            double t_update = omp_get_wtime();
            double update_time = (t_update - t_update_0) * 1000;

            update_time_vec.push_back(update_time);
            iter_counts.push_back(iter_num + 1);

            cout << "update time: " << update_time << " ms" << endl;
            all_update_time += update_time;
            // cout << "predict estimation time: " << (t_update - t_pred) * 1000 << " ms" << endl;
            state_point = kf.get_x();
            pos_lid = state_point.pos + state_point.rot.matrix() * state_point.offset_T_L_I;
            ...
            // double t_map_0 = omp_get_wtime();
            map_incremental();
            // double t_map_1 = omp_get_wtime();
            // cout << "update time: " << (t_update - t_update_0) * 1000 << " ms" << endl;
            // cout << "map_incremental time: " << (t_map_1 - t_map_0) * 1000 << " ms" << endl;
    ...
    /**************** save traj ****************/
    printf("all_frames: %d, all_time(ms): %0.6f, average_time(ms): %0.6f\n, average_time(ms): %0.6f\n", frame_count, all_time, all_time / frame_count, all_update_time / frame_count);
    // cout << "*******************average_time(ms)************************* " << all_time / frame_count << endl;
    // string traj_file = "/home/zj/workspace/zc_paper_ws/traj_compare/inva-fast-lio-out/traj.txt";
    cout << "finishing mapping" << endl;
    std::ofstream ofs;
    ofs.open(traj_log_file, std::ios::out);
    if (!ofs.is_open()) {
        cout << "Failed to open traj_file: " << traj_log_file;
        return 0;
    }
    ...
    string iter_count_file = "/home/zj/workspace/zc_paper_ws/final_figure/inva_lio_iter_count.txt";
    std::ofstream ofs_1;
    ofs_1.open(iter_count_file, std::ios::out);
    if (!ofs_1.is_open()) {
        cout << "Failed to open iter_count_file: " << iter_count_file;
        return 0;
    }
    for (const auto &cnt : iter_counts) {
        ofs_1 << cnt << std::endl;
    }
    ofs_1.close();

    std::ofstream ofs_2;
    string update_time_count_file = "/home/zj/workspace/zc_paper_ws/final_figure/inva_lio_average_update_time.txt";
    ofs_2.open(update_time_count_file, std::ios::out);
    if (!ofs_2.is_open()) {
        cout << "Failed to open traj_file: " << update_time_count_file;
        return 0;
    }
    // ofs << "#timestamp x y z q_x q_y q_z q_w" << std::endl;
    for (const auto &p : update_time_vec) {
        ofs_2 << std::fixed << std::setprecision(6) << p << std::endl;
    }
    ofs_2.close();
